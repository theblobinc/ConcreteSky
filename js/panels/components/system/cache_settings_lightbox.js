  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        :host{position:fixed;inset:0;z-index:100001;display:${opened ? 'flex' : 'none'};align-items:center;justify-content:center;font-family: system-ui, -apple-system, Segoe UI, Roboto, Arial, sans-serif;}
        .backdrop{position:absolute;inset:0;background:rgba(0,0,0,.72)}
        .card{position:relative;width:min(980px, calc(100vw - 20px));max-height:min(86vh, 980px);overflow:auto;background:#0b0b0b;border:1px solid #2b2b2b;border-radius: var(--bsky-radius, 0px);box-shadow:0 18px 60px rgba(0,0,0,.65);color:#fff}
        ...
        .tab{appearance:none;background:#111;color:#fff;border:1px solid #333;border-radius: var(--bsky-radius, 0px);padding:8px 12px;cursor:pointer;font-weight:800}
        .tab[aria-pressed="true"]{background:#1d2a41;border-color:#2f4b7a}
        .close{appearance:none;background:#111;color:#fff;border:1px solid #333;border-radius: var(--bsky-radius, 0px);padding:8px 12px;cursor:pointer;font-weight:900}
        .close:hover,.tab:hover{background:#1b1b1b}
        .bd{padding:12px 14px}
        .note{color:#aaa;margin:0 0 10px 0}
      </style>
