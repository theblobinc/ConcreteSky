...
class BskyFollowing extends HTMLElement {
  constructor(){
    super();
  ...
  render(){
    ...
    const listInner = (String(this.view || 'list') === 'list')
      ? this._winCtl.innerHtml({
          ...
          emptyHtml: '<div class="muted">No following loaded.</div>',
        })
      : (rows || (this.loading ? '<div class="muted">Loading…</div>' : '<div class="muted">No following loaded.</div>'));
...
customElements.define('bsky-following', BskyFollowing);
